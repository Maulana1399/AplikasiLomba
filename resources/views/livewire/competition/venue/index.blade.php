<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Venue</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Kelola lokasi/arena perlombaan.</p>
        </div>
        <flux:button wire:click="toggleCreateForm" variant="primary">
            {{ $showCreateForm ? 'Batal' : 'Tambah Venue' }}
        </flux:button>
    </div>
    ...
    @if ($showCreateForm)
        <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
            <h2 class="mb-4 text-lg font-semibold text-zinc-900 dark:text-white">Venue Baru</h2>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Nama Venue</label>
                    <flux:input wire:model="newName" placeholder="Nama venue" />
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Keterangan/Lokasi</label>
                    <flux:input wire:model="newLocationDetail" placeholder="Detail lokasi (opsional)" />
                    @error('newLocationDetail') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            <thead class="bg-zinc-50 dark:bg-zinc-900">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500">Keterangan/Lokasi</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($venues as $venue)
                    @if ($editId === $venue->id)
                        <tr class="bg-amber-50 dark:bg-amber-950/20">
                            ...
                            <td class="px-4 py-2">
                                <flux:input wire:model="editLocationDetail" size="sm" />
                            </td>
                            <td class="px-4 py-2">
                                <flux:input wire:model="editSortOrder" type="number" size="sm" />
                            </td>
                            <td class="px-4 py-2 text-sm">{{ $venue->is_active ? 'Aktif' : 'Nonaktif' }}</td>
                            <td class="px-4 py-2">
                    ...
                    @else
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                            <td class="px-4 py-3 text-sm font-medium text-zinc-900 dark:text-white">{{ $venue->name }}</td>
                            <td class="px-4 py-3 text-sm text-zinc-500">{{ $venue->code ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-zinc-500">{{ $venue->location_detail ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-zinc-500">{{ $venue->sort_order ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm">
                                <span @class([
                                    'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium',
                                    'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' => $venue->is_active,
                                    'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200' => !$venue->is_active,
                                ])>{{ $venue->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex gap-1">
                                    <flux:button wire:click="edit({{ $venue->id }})" size="sm" icon-trailing="pencil">Edit</flux:button>
                                    <flux:button wire:click="toggleActive({{ $venue->id }})" size="sm" variant="{{ $venue->is_active ? 'danger' : 'primary' }}">
                                        {{ $venue->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                    </flux:button>
                                    <flux:button wire:click="delete({{ $venue->id }})" size="sm" variant="danger"
                                        wire:confirm="Yakin hapus venue ini? Tindakan tidak dapat dibatalkan.">Hapus</flux:button>
                                </div>
                            </td>
                        </tr>
                    @endif
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-zinc-500">Belum ada venue.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
