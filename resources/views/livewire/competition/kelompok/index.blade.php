<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Kelompok</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Master data kelompok per desa yang ditawarkan pada form registrasi.</p>
        </div>
        <flux:button wire:click="toggleCreateForm" variant="primary">
            {{ $showCreateForm ? 'Batal' : 'Tambah Kelompok' }}
        </flux:button>
    </div>
    ...
    @if ($showCreateForm)
        <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
            <h2 class="mb-4 text-lg font-semibold text-zinc-900 dark:text-white">Kelompok Baru</h2>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Nama Kelompok</label>
                    <flux:input wire:model="newName" placeholder="Nama kelompok" />
                ...
                <div>
                    ...
                    <flux:select wire:model="newDesaId" placeholder="Pilih desa">
                        @foreach ($desas as $d)
                            <flux:select.option value="{{ $d->id }}">{{ $d->desa_asal }}</flux:select.option>
                        @endforeach
                    </flux:select>
                    @error('newDesaId') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
    ...
    <div class="overflow-x-auto rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            <thead class="bg-zinc-50 dark:bg-zinc-900">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500">Nama Kelompok</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($kelompoks as $k)
                    @if ($editId === $k->id)
                        <tr class="bg-amber-50 dark:bg-amber-950/20">
                            ...
                            <td class="px-4 py-2">
                                <flux:select wire:model="editDesaId" size="sm">
                                    @foreach ($desas as $d)
                                        <flux:select.option value="{{ $d->id }}">{{ $d->desa_asal }}</flux:select.option>
                                    @endforeach
                                </flux:select>
                                @error('editDesaId') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </td>
                            <td class="px-4 py-2">
                                <flux:input wire:model="editSortOrder" type="number" size="sm" />
                            </td>
                            <td class="px-4 py-2 text-sm">
                                <div class="text-sm text-zinc-900 dark:text-zinc-100">{{ $k->is_active ? 'Aktif' : 'Nonaktif' }}</div>
                            </td>
                            <td class="px-4 py-2">
                    ...
                    @else
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                            <td class="px-4 py-3 text-sm font-medium text-zinc-900 dark:text-white">{{ $k->kelompok_asal }}</td>
                            <td class="px-4 py-3 text-sm text-zinc-500">{{ $k->desa?->desa_asal ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-zinc-500">{{ $k->sort_order ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm">
                                <span @class([
                                    'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium',
                                    'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' => $k->is_active,
                                    'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200' => !$k->is_active,
                                ])>{{ $k->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex gap-1">
                                    <flux:button wire:click="edit({{ $k->id }})" size="sm" icon-trailing="pencil">Edit</flux:button>
                                    <flux:button wire:click="toggleActive({{ $k->id }})" size="sm" variant="{{ $k->is_active ? 'danger' : 'primary' }}">
                                        {{ $k->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                    </flux:button>
                                    <flux:button wire:click="delete({{ $k->id }})" size="sm" variant="danger"
                                        wire:confirm="Yakin hapus kelompok ini? Tindakan tidak dapat dibatalkan.">Hapus</flux:button>
                                </div>
                            </td>
                        </tr>
                    @endif
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-sm text-zinc-500">Belum ada kelompok.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
